...
const ShowNote = ({ item }) => {
    ...
    return (
        <Link to={`/note/${item._id}`} className={`notesData group ${view ? `w-full` : `w-[60%]`} flex flex-wrap justify-center content-between h-auto ${view ? `max-h-[32rem]` : `max-h-[42rem]`} min-h-24 border relative border-[rgb(95,99,104)] rounded-lg mb-4`}>
            ...
            <div className={`${view ? `px-1` : `px-8`} pb-1 noteController flex items-center ${!item.isBin ? `justify-between` : `justify-start`} w-full h-[2.125rem] sticky bottom-0 left-0 opacity-0 group-hover:opacity-100 transition-opacity`}>
